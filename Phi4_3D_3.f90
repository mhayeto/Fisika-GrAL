...
! Restrictions: cubic lattice (z=6).
 ...
 program Phi4_3D
  ...
  ! Parameters
    integer, parameter :: L=10, sps=5E6, therm=1E4     ! Number of sites in one dimension, Steps per site, Therm. steps
    real, parameter :: d=0.45, CE0=1.0                 ! Width param, Coupling consty/Energy barrier for local wells
    real, parameter :: Tf=5.5, dT=0.05                 ! Final temp, Temp. step
  ! Variables
...
! "Performs one 'sweep' of the lattice, i.e., one Monte Carlo step/spin, using Metropolis algorithm."
     ...
     Lattice_sweep: do i = 0, N
      ...
      ! Decide wether to change value from xi to xi + dx: dE positive or negative?
        ...
        elseif (r < exp(-dE*beta) ) then ! dE without minus sign because we take as dE=Ei-Ef=-(Ef-Ei)
           X(rand_int) = xi + dx
