...
! Version: 1.5
! Description: 
!               Ising 2D model (B=0), from "Monte Carlo Methods in Statistical Physics" book (pages
...
!
! Restrictions: square lattice (z=4), 2D, B=0, single-spin-flip. 
 ...
 program Ising2D
  ...
  ! Variables
    ...
    integer, dimension(0:L*L-1) :: S                  ! L*L array containing spins (1D because of helical b.c.)
    ...
    real, dimension(2) :: prob                        ! Array containing possible acceptance ratios
...
! that the lattice is square and thus z=4. Then it can be seen that there are just two possible values.

     real, intent(in) :: beta, J
     real, dimension(2), intent(out) :: prob

     prob(1) = exp(-8.0*beta*J)
     prob(2) = exp(-4.0*beta*J)
...
! the number of nearest neighbours (z) is 4. Helical boundary conditions are implemented here. 
     ...
     spin_sum = S(modulo(i+1, N+1)) + S(modulo(i-1, N+1)) + S(modulo(i+L, N+1)) + S(modulo(i-L, N+1))
...
! It returns the energy difference (deltaE) between the previous and the next states.  
     ...
     integer, dimension(0:N), intent(in out) :: S
     real, dimension(2), intent(in) :: prob
